...
from .schemas import CalculerParcoursRequest, ParcoursResponse, PointResponse
...
def calculate(
    ...
    try:
        ...
        # Le dernier tracé valide (s'il existe) reste affiché côté client :
        # cette route ne fait qu'échouer proprement, elle n'écrase rien.
        raise AppError(
            502,
    ...
    # Jamais de géométrie exposée hors statut "routed" -- même garde que
    # `PostgisRouteRepository.save` (`geometry=None` si non routé) : un
    # fournisseur pourrait renvoyer une forme dégénérée (ex. un seul point
    # pour un départ == destination) sans que `unrouted_points` la
    # signale ; ce n'est pas un tracé exploitable, jamais affiché comme tel.
    geometry = route.result.geometry if route.statut == STATUT_ROUTE else ()
